 ...
 */
template <class TransformFunc, int N, class StateDist, int M, class ResultDist, class NoiseMatrix>
...
void unscented_update (const unscented_params<N>& params, ObsFunc h,
    ...
    unscented_transform (params, h, state, predicted, obs.derived().chol_cov(), &kalman_gain);
    ...
    for (int i = 0; i < M; ++i) cholesky_downdate<N> (state.chol_cov(), kalman_gain.col(i));
}
