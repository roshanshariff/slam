...
cowtree::editor::editor(root& tree) : parent_ptr(nullptr), subtree_ptr(&tree) {
  subtree_ptr->make_unique();
...
cowtree::editor::~editor() {
  if (!subtree_ptr->is_black()) {
    if (is_root()) { subtree_ptr->set_black(true); }
    else if (!parent_ptr->subtree_ptr->is_black()) {
      if (is_left_child() != parent_ptr->is_left_child()) { rotate(); }
