...
public:
  ...
  auto operator==(const random_source& r) const -> bool {
...
public:
  auto likelihood(const vector_type& x) const -> double {
    ...
    return std::exp(likelihood_exponent(x)) * std::pow(root_two_pi, -vector_dim)
           / chol_cov_det();
  }

  auto log_likelihood(const vector_type& x) const -> double {
    const double log_root_two_pi =
        0.5 * std::log(2 * boost::math::constants::pi<double>());
    return likelihood_exponent(x) - vector_dim * log_root_two_pi
           - chol_cov_log_det();
  }
...
template <int N, class Derived>
struct multivariate_normal_dense_base
    : public multivariate_normal_base<N, Derived> {
...
protected:
  ...
  multivariate_normal_dense_base(const vector_type& mean,
                                 const matrix_type& chol_cov)
...
public:
  ...
  void chol_cov_trans_solve(vector_type& v) const {
    m_chol_cov.template triangularView<Eigen::Lower>().transpose().solveInPlace(
        v);
  }
...
template <int N>
struct multivariate_normal_dist
    : public multivariate_normal_dense_base<N, multivariate_normal_dist<N>> {
...
struct multivariate_normal_adapter
    : public multivariate_normal_dense_base<Adapted::vector_dim,
                                            multivariate_normal_adapter<Adapted>> {

  using base_type = multivariate_normal_dense_base<Adapted::vector_dim,
  ...
  multivariate_normal_adapter(const vector_type& mean,
                              const matrix_type& chol_cov)
      : base_type(mean, chol_cov) {}
