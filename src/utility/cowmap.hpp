...
public:
  ...
  cowmap(key_compare cmp = key_compare())
      : data(cowtree::root(), value_compare(cmp)) {}
  ...
  void for_each(Functor&& f) const {
    inorder_traverse(root(), std::forward<Functor>(f));
  }
  ...
  size_type count(const key_type& key) const {
    return find_subtree(key).empty() ? 0 : 1;
  }
...
private:
  ...
  template <class Functor>
  void inorder_traverse(const cowtree&, Functor&&) const;
...
const cowtree& cowmap<K, V, Compare>::find_subtree(const key_type& key) const {
  ...
  while (!subtree->empty()) {
    if (key_comp()(key, subtree->value<value_type>().first))
      subtree = &subtree->left();
    else if (key_comp()(subtree->value<value_type>().first, key))
...
template <class K, class V, class Compare>
bool cowmap<K, V, Compare>::insert(const value_type& entry,
...
template <class K, class V, class Compare>
template <class Functor>
void cowmap<K, V, Compare>::inorder_traverse(const cowtree& subtree,
                                             Functor&& f) const {
  if (!subtree.empty()) {
