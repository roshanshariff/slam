...
using slam_result_type =
    slam::slam_result_of<control_model_type, observation_model_type>;
using slam_result_impl_type =
    slam::slam_result_of_impl<control_model_type, observation_model_type>;

using simulator_type = simulator<control_model_type, observation_model_type,
                                 controller_type, sensor_type>;
using slam_data_type =
    slam::slam_data<control_model_type, observation_model_type>;
using slam_dataset_type =
    slam::dataset<control_model_type, observation_model_type>;
using slam_initialiser_type =
    slam::slam_initialiser<control_model_type, observation_model_type>;
using mcmc_slam_type =
    slam::mcmc_slam<control_model_type, observation_model_type>;
using multi_mcmc_type =
    slam::multi_mcmc<control_model_type, observation_model_type>;
using g2o_slam_type =
    slam::g2o_slam<control_model_type, observation_model_type>;
using g2o_clustering_type =
    slam::g2o_clustering<control_model_type, observation_model_type>;
...
template <class T>
T remember_option(boost::program_options::variables_map& options,
                  const std::string& key, T default_value) {
  if (options[key].empty()) {
    options.insert(std::make_pair(
        key, boost::program_options::variable_value(default_value, false)));
...
/*
void learn_model (const slam_dataset_type& dataset, const slam_result_type&
ground_truth,
                  const boost::program_options::variables_map& options);
*/

void dump_data(const slam_result_type& ground_truth,
               const slam_result_type& estimate, const std::string& prefix) {

  using planar_robot::pose;
  using planar_robot::position;

  pose origin = planar_robot::estimate_initial_pose(
      ground_truth.get_feature_map(), estimate.get_feature_map());
...
int main(int argc, char* argv[]) {
  ...
  random_source random(
      remember_option(options, "seed", (unsigned int)std::random_device()()));

  unsigned int sim_seed =
      remember_option(options, "sim-seed", (unsigned int)random());
  unsigned int init_seed =
      remember_option(options, "init-seed", (unsigned int)random());
  unsigned int mcmc_slam_seed =
      remember_option(options, "mcmc-slam-seed", (unsigned int)random());
  unsigned int hmc_seed =
      remember_option(options, "hmc-seed", (unsigned int)random());
  unsigned int fastslam_seed =
      remember_option(options, "fastslam-seed", (unsigned int)random());
  unsigned int multi_mcmc_seed =
      remember_option(options, "multi-mcmc-seed", (unsigned int)random());
  ...
  if (options.count("dataset")) {
    std::tie(dataset, ground_truth) =
        read_range_only_data(options["dataset-dir"].as<std::string>(),
                             options["dataset"].as<std::string>());
  }
  else {
    controller_type controller(options);
    sensor_type sensor(options);
    auto simulator = std::make_shared<simulator_type>(
        options, sim_seed, control_model_builder, observation_model_builder,
        controller, sensor);
    dataset = simulator;
  ...
  */
  ...
  if (options.count("mcmc-slam")) {
    ...
    mcmc_slam_updater =
        std::make_shared<mcmc_slam_type::updater>(mcmc_slam, options);
  ...
  if (options.count("multi-mcmc")) {
    multi_mcmc =
        std::make_shared<multi_mcmc_type>(data, options, multi_mcmc_seed);
  ...
  if (options.count("g2o")) {
    ...
    g2o_slam_updater =
        std::make_shared<g2o_slam_type::updater>(g2o_slam, options);
  ...
  if (options.count("plot-stats")) {
    ...
    if (mcmc_slam) {
      ...
      likelihood_plot->add_data_source(
          [=](slam::timestep_type) {
            return slam::slam_log_likelihood(*data, *mcmc_slam)
                   - slam::slam_log_likelihood(*data, *ground_truth);
          },
...

  {
    ...
    data->add_dataset(*dataset, control_model_builder,
  ...
  if (options.count("slam-plot")) {
    ...
    if (mcmc_slam) {
      slam_plot->add_data_source(
          mcmc_slam, false, "MCMC-SLAM", "", "lc rgbcolor 'blue' pt 3 ps 3",
    ...
    if (g2o_slam) {
      slam_plot->add_data_source(
          g2o_slam, false, "G2O", "", "lc rgbcolor 'red' pt 3 ps 1",
          "lc rgbcolor 'red' lw 2", "size 10,20,50 filled lc rgbcolor 'red'");
    }

    data->add_timestep_listener(slam_plot);
  }

  const double dataset_log_likelihood =
      slam::slam_log_likelihood(*data, *ground_truth);
  ...
  const auto print_rmse = [ground_truth](const slam_result_type& estimate,
                                         const char* name) {

    double map_rmse, traj_rmse;

    std::tie(map_rmse, traj_rmse) =
        planar_robot::map_traj_rmse_align_start(*ground_truth, estimate);
    std::cout << name << " Map RMSE: " << map_rmse << '\n'
              << name << " Trajectory RMSE: " << traj_rmse << '\n';

    std::tie(map_rmse, traj_rmse) =
        planar_robot::map_traj_rmse_align_best(*ground_truth, estimate);
  ...
  if (mcmc_slam) {
    ...
    std::cout << "MCMC-SLAM log likelihood ratio: "
  ...
  if (multi_mcmc) {
    ...
    std::cout << "Multi-MCMC-SLAM Chains: " << multi_mcmc->num_chains() << '\n';
    print_rmse(*mcmc_slam, "Multi-MCMC");
    std::cout << "Multi-MCMC-SLAM log likelihood ratio: "
    ...
    std::cout << "Averaged-Multi-MCMC log likelihood ratio: "
              << slam::slam_log_likelihood(*data, *average)
  ...
  if (g2o_slam) {
    ...
    std::cout << "G2O-SLAM log likelihood ratio: "
              << slam::slam_log_likelihood(*data, *g2o_slam)
                     - dataset_log_likelihood
              << "\n\n";
  }
  ...
  if (mcmc_slam && g2o_clustering) {
    ...
    std::vector<std::string> color_names = {"red",  "green",   "blue",
                                            "cyan", "magenta", "yellow"};
    ...
    for (const auto& cluster : g2o_clustering->get_clusters()) {
      ...
      top_clusters.push_back(
          std::make_shared<slam_result_impl_type>(cluster.estimate));
      std::cout << top_clusters.size() << ": "
                << (cluster.log_likelihood - dataset_log_likelihood) << '\n';
    }
    ...
    for (size_t i = 0; i < top_clusters.size(); ++i) {
      cluster_plot->add_data_source(
          top_clusters[i], false, "", "",
  ...
  if (options.count("hmc")) {
    ...
    if (slam_plot) slam_plot->completed();
    for (int i = 1; i < 10; ++i) {
      if (hmc->update(1e-10, 10)) { std::cout << "ACCEPTED\n"; }
      else {
        std::cout << "REJECTED\n";
      }
      if (slam_plot) slam_plot->completed();
...
boost::program_options::variables_map parse_options(int argc, char* argv[]) {
  ...
  command_line_options.add_options()  //
      ...
      ("config-file,f", po::value<std::vector<std::string>>()->composing(),
       "configuration files");

  po::options_description general_options("General Options");
  general_options.add_options()("dataset", po::value<std::string>(),
                                "name of dataset") //
      ("dataset-dir", po::value<std::string>()->default_value("input"),
      ...
      ("hmc", "use Hamiltonian MC algorithm")                       //
      ...
      ("dump-data", po::value<std::string>()->default_value(""),
       "dump MCMC-SLAM results") //
      ("seed", po::value<unsigned int>(),
       "seed for global random number generator");

  const std::vector<std::pair<std::string, po::options_description>>
      module_options{
          ...
          {"Observation Model",
           observation_model_type::builder::program_options()},
  ...
  auto module_option_desc =
    ...
    std::transform(option_help.begin(), option_help.end(), option_help.begin(),
                   (int (*)(int))std::tolower);
  ...
  for (const auto& module : module_options) {
    ...
    command_line_options.add_options()(option_desc.first.c_str(),
                                       option_desc.second.c_str());
  ...
  if (values.count("config-file")) {
    std::vector<std::string> files =
        values["config-file"].as<std::vector<std::string>>();
...
ground_truth,
    ...
    //observation_model_builder = observation_model_type::learn_from_data
...
observation_model_builder);
        ...
        control_model_builder = control_model_type::learn_from_data (dataset,
*mcmc_slam);
    }
