...
public:
    ...
    struct data_source {
        ...
        data_source (std::size_t capacity, const function_type& f, const std::string& title,
                     const std::string& style, bool secondary)
        : history(capacity), function(f), title(title), style(style), secondary_axis(secondary) { }
    };
    ...
    void add_data_source (const function_type& f, const std::string& title, const std::string& style,
                          bool secondary_axis = false) {
        
        data_sources.push_back (data_source (history_capacity, f, title, style, secondary_axis));
    }
