...
void slam_plotter::add_data_source(std::shared_ptr<slam_result_type> source,
                                   bool autoscale_map,
  ...
  data_sources.push_back({source, autoscale_map, trajectory_title,
                          landmark_title, feature_point_style,
                          trajectory_line_style, state_arrow_style});
}

void slam_plotter::timestep(slam::timestep_type t) {
  if (output_dir) {
    std::ostringstream output_filename;
    output_filename << std::setfill('0') << std::setw(6) << std::size_t(t)
                    << ".png";
...
void slam_plotter::plot(boost::optional<slam::timestep_type> timestep) {
  ...
  if (!title.empty())
    std::fprintf(gnuplot.handle(), "set title '%s'\n", title.c_str());
  else
    gnuplot.puts("set title\n");
  ...
  for (const auto& source : data_sources) {
    ...
    if (ground_truth && source.source != ground_truth) {
      if (match_ground_truth && source.source->get_feature_map().size() >= 2) {
        origin = planar_robot::estimate_initial_pose(
            ground_truth->get_feature_map(), source.source->get_feature_map());
      }
      else {
        origin = ground_truth->get_initial_state()
                 + (-source.source->get_initial_state());
      }
    }
...
void slam_plotter::add_title(const std::string& title) {
  if (!title.empty())
    std::fprintf(gnuplot.handle(), "title '%s' ", title.c_str());
  else
...
void slam_plotter::plot_trajectory(const data_source& source,
                                   slam::timestep_type t, const pose& origin) {
...
boost::program_options::options_description slam_plotter::program_options() {
  ...
  options.add_options() //
      ("slam-plot-title", po::value<std::string>()->default_value(
                              "Simultaneous Localization and Mapping"),
       "Plot title") //
      ("slam-plot-output-dir", po::value<std::string>(),
       "Output directory for plots (displayed on screen if unset)") //
      ("slam-plot-isometry",
       "calculate best fit between estimated map and ground truth") //
...
slam_plotter::slam_plotter(boost::program_options::variables_map& options)
  ...
  if (options.count("slam-plot-output-dir")) {
    output_dir = boost::filesystem::path(
        options["slam-plot-output-dir"].as<std::string>());
