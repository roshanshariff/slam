...
template <class ControlModel, class ObservationModel, class Controller,
          class Sensor>
class simulator
  ...
  using slam_result_impl_type =
      slam::slam_result_of_impl<ControlModel, ObservationModel>;
...
public:
  ...
  virtual auto current_timestep() const -> slam::timestep_type override {
    return slam::dataset_impl<ControlModel,
                              ObservationModel>::current_timestep();
  }
};

template <class ControlModel, class ObservationModel, class Controller,
          class Sensor>
simulator<ControlModel, ObservationModel, Controller, Sensor>::simulator(
  ...
  auto sense = [&]() {
    ...
    sensor.template sense<ObservationModel>(
        observation_model_builder, random, current_state(),
  ...
  while (!controller.finished()) {
    ...
    const auto control =
        controller.template control<ControlModel>(current_state(), dt);
...
simulator<ControlModel, ObservationModel, Controller,
          Sensor>::program_options() {
  ...
  options.add_options() //
      ...
      ("sim-seed", po::value<unsigned int>(),
       "Seed for control and observation noise") //
      ("sensor-skip", po::value<unsigned int>()->default_value(8),
       "Take sensor readings every n time steps");
  return options;
}

extern template class simulator<control_model_type, observation_model_type,
