...
gnuplot_process::gnuplot_process(bool debug) : debug_mode(debug) {
  if (!debug_mode)
    fh = popen("gnuplot -p", "w");
  else
    fh = fopen("debug-gnuplot.txt", "w");
}
...
void gnuplot_process::plot(size_t columns) {
  ...
  if (records > 0) {
    ...
    if (columns > 1)
      std::fprintf(fh, "'-' binary record=%zu format='%%%zufloat' ", records,
                   columns);
    else
      std::fprintf(fh, "'-' binary array=%zu format='%%float' ", records);
  }
}
