...
auto read_range_only_data(boost::filesystem::path dir, const std::string& name)
  ...
  /** Ground truth path from GPS */
  {
    ...
    while (input_GT >> t >> x >> y >> bearing) {
      auto state =
          -initial_state + planar_robot::pose::cartesian(x, y, bearing);
  ...
  using dataset_impl_type =
      slam::dataset_impl<control_model_type, observation_model_type>;
  ...
  /** Node range observations */
  {
    ...
    while (input_TD >> timestamp >> antenna >> id >> observation(0)) {
      ...
      slam::timestep_type timestep =
          dataset->timestep_at(timestamp - start_timestamp);
...

  {
    ...
    controls
        << "# dt, actual v, actual w, actual g, commanded v, commanded w\n";
