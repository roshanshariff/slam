...
class g2o_slam : public slam_result_of<ControlModel, ObservationModel>,
  ...
  /** See http://gcc.gnu.org/bugzilla/show_bug.cgi?id=58047 for why the
   *redundant
  ...
  struct vertex_state
      : public g2o::BaseVertex<state_type::vector_dim, state_type> {
  ...
  protected:
    virtual void setToOriginImpl() override { this->setEstimate(state_type()); }

    virtual void oplusImpl(const double* update) override {
      using map_type = Eigen::Map<const typename state_type::vector_type>;
      this->setEstimate(this->estimate()
                        + state_type::from_vector(map_type(update)));
    }
  };

  struct vertex_landmark
      : public g2o::BaseVertex<feature_type::vector_dim, feature_type> {
  ...
  protected:
    ...
    virtual void oplusImpl(const double* update) override {
      ...
      this->setEstimate(feature_type::from_vector(this->estimate().to_vector()
                                                  + map_type(update)));
    }
  };

  struct edge_control
      : public g2o::BaseBinaryEdge<ControlModel::vector_dim, control_type,
                                   vertex_state, vertex_state> {

    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;

    edge_control(vertex_state* v0, vertex_state* v1,
      ...
      control.chol_cov().template triangularView<Eigen::Lower>().solveInPlace(
          chol_cov_inv);
  ...
  protected:
    void computeError() override {
      ...
      control_type predicted =
          ControlModel::observe(-v1->estimate() + v2->estimate());
      this->error() = ControlModel::subtract(predicted, this->measurement());
    }
  };

  struct edge_obs : public g2o::BaseBinaryEdge<ObservationModel::vector_dim,
                                               observation_type, vertex_state,
                                               vertex_landmark> {

    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;

    edge_obs(vertex_state* v0, vertex_landmark* v1,
      ...
      chol_cov_inv.setIdentity();
      obs.chol_cov().template triangularView<Eigen::Lower>().solveInPlace(
          chol_cov_inv);
      this->setInformation(chol_cov_inv.transpose() * chol_cov_inv);
    }
  ...
  protected:
    void computeError() override {
      ...
      observation_type predicted =
          ObservationModel::observe(-v1->estimate() + v2->estimate());
      this->error() =
          ObservationModel::subtract(predicted, this->measurement());
    }
  };
...
template <class ControlModel, class ObservationModel>
void slam::g2o_slam<ControlModel, ObservationModel>::reinitialise(
    const slam_result_type& initialiser) {
  ...
  for (const auto& feature : initialiser.get_feature_map()) {
    feature_vertices.at(feature.first)
        ->setEstimate(-initial_state + feature.second);
  }
  map_estimate.clear();
}

template <class ControlModel, class ObservationModel>
auto slam::g2o_slam<ControlModel, ObservationModel>::optimise(
    const int max_iterations) -> int {

  if (max_iterations <= 0 || state_vertices.size() <= 1
...
template <class ControlModel, class ObservationModel>
void slam::g2o_slam<ControlModel, ObservationModel>::control(
    timestep_type t, const ControlModel& control) {

  assert(t == current_timestep());
  assert(t == state_vertices.size() - 1);
  assert(initialiser);

  state_type initial_estimate =
      -initialiser->get_state(t) + initialiser->get_state(t + 1);

  auto v0 = state_vertices.back();
  auto v1 = utility::make_unique<vertex_state>(
      next_vertex_id++, v0->estimate() + initial_estimate);
...
template <class ControlModel, class ObservationModel>
void slam::g2o_slam<ControlModel, ObservationModel>::observation(
    timestep_type t, const typename slam_data_type::observation_info& obs) {
  ...
  if (obs.index() == 0) {
    ...
    assert(initialiser);
    feature_type initial_estimate =
        -initialiser->get_state(t) + initialiser->get_feature(obs.id());

    auto v = utility::make_unique<vertex_landmark>(
        next_vertex_id++, v0->estimate() + initial_estimate);
...
template <class ControlModel, class ObservationModel>
void slam::g2o_slam<ControlModel, ObservationModel>::timestep(timestep_type t) {
...
template <class ControlModel, class ObservationModel>
auto slam::g2o_slam<ControlModel, ObservationModel>::get_trajectory() const
...
template <class ControlModel, class ObservationModel>
auto slam::g2o_slam<ControlModel, ObservationModel>::get_feature_map() const
  ...
  if (map_estimate.size() != feature_vertices.size()) {
    ...
    for (const auto& entry : feature_vertices) {
      map_estimate.emplace_hint(map_estimate.end(), entry.first,
...
template <class ControlModel, class ObservationModel>
slam::g2o_slam<ControlModel, ObservationModel>::g2o_slam(
    const decltype(initialiser)& init)
    : initialiser(init) {

  using SlamBlockSolver = g2o::BlockSolver<g2o::BlockSolverTraits<-1, -1>>;
  using SlamLinearSolver =
      g2o::LinearSolverCSparse<SlamBlockSolver::PoseMatrixType>;
  ...
  auto blockSolver =
      utility::make_unique<SlamBlockSolver>(linearSolver.release());

  auto solver =
      utility::make_unique<OptimizationAlgorithm>(blockSolver.release());
...
template <class ControlModel, class ObservationModel>
auto slam::g2o_slam<ControlModel, ObservationModel>::program_options()
    -> boost::program_options::options_description {
  namespace po = boost::program_options;
  po::options_description options("G2O-SLAM Parameters");
  options.add_options() //
      ("g2o-steps", po::value<unsigned int>()->default_value(0),
       "G2O iterations per time step") //
      ("g2o-end-steps", po::value<unsigned int>()->default_value(0),
       "G2O iterations after simulation");
  return options;
}

template <class ControlModel, class ObservationModel>
slam::g2o_slam<ControlModel, ObservationModel>::updater::updater(
    const decltype(instance)& instance,
    const boost::program_options::variables_map& options)
    : updater(instance, options["g2o-steps"].as<unsigned int>(),
              options["g2o-end-steps"].as<unsigned int>()) {}

extern template class slam::g2o_slam<control_model_type,
