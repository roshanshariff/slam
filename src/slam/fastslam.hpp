...
template <class ControlModel, class ObservationModel>
class fastslam : public slam_result_of<ControlModel, ObservationModel>,
...
private:
  ...
  using state_dist =
      vector_model_adapter<multivariate_normal_adapter<state_type>>;
  using feature_dist =
      vector_model_adapter<multivariate_normal_adapter<feature_type>>;
  ...
  /** Current timestep, current control, and observations made in the current
   * timestep. */
  ...
  /** Whether to keep a per-particle trajectory as opposed to one combined
   * trajectory. */
  ...
  const struct unscented_params_holder {
    ...
    unscented_params_holder(double alpha, double beta, double kappa)
        : control(alpha, beta, kappa), obs(alpha, beta, kappa),
          feature(alpha, beta, kappa), state_feature(alpha, beta, kappa) {}
  } ukf_params;
...
template <class ControlModel, class ObservationModel>
void slam::fastslam<ControlModel, ObservationModel>::timestep(
  ...
  if (timestep > 0) {
    ...
    const state_type& state_estimate =
        particles.max_weight_particle().trajectory.state;
  ...
  for (const auto& obs : seen_features) {
    for (auto& particle : particles) {
      ...
      unscented_update(ukf_params.feature,
                       typename vec::feature_observer(state),
  ...
  for (const auto& obs : new_features) {
    for (auto& particle : particles) {
      ...
      unscented_transform(ukf_params.obs,
                          typename vec::feature_initializer(state),
  ...
  std::cout << "Effective particle set: " << particles.effective_size()
            << std::endl;
}

template <class ControlModel, class ObservationModel>
auto slam::fastslam<ControlModel, ObservationModel>::particle_state_update(
    particle_type& particle) const -> double {

  state_dist state, state_proposal;

  unscented_transform(ukf_params.control,
                      typename vec::state_predictor(particle.trajectory.state),
                      *current_control, state.vector_model());

  { // Calculate proposal distribution

    multivariate_normal_dist<vec::state_dim + vec::feature_dim>
        state_feature_joint;
    state_feature_joint.mean().template head<vec::state_dim>() =
        state.vector_model().mean();
    state_feature_joint.chol_cov()
        .template topLeftCorner<vec::state_dim, vec::state_dim>() =
        state.vector_model().chol_cov();

    for (const auto& obs : seen_features) {

      const feature_dist& feature = particle.features.get(obs.id);

      state_feature_joint.mean().template tail<vec::feature_dim>() =
          feature.vector_model().mean();
      state_feature_joint.chol_cov()
          .template bottomRightCorner<vec::feature_dim, vec::feature_dim>() =
          feature.vector_model().chol_cov();

      state_feature_joint.chol_cov()
          .template topRightCorner<vec::state_dim, vec::feature_dim>()
          .setZero();
      state_feature_joint.chol_cov()
          .template bottomLeftCorner<vec::feature_dim, vec::state_dim>()
          .setZero();

      unscented_update(ukf_params.state_feature,
                       typename vec::state_feature_observer(),
                       state_feature_joint, obs.observation);
    }

    state_proposal.vector_model().mean() =
        state_feature_joint.mean().template head<vec::state_dim>();
    state_proposal.vector_model().chol_cov() =
        state_feature_joint.chol_cov()
            .template topLeftCorner<vec::state_dim, vec::state_dim>();
  }
  ...
  if (!discard_history) {
    particle.trajectory.previous =
        std::make_shared<typename particle_type::state_list>(
            particle.trajectory);
  }
  ...
  const double state_log_likelihood =
      state.log_likelihood(particle.trajectory.state);
  const double proposal_log_likelihood =
      state_proposal.log_likelihood(particle.trajectory.state);

  return std::exp(obs_log_likelihood + state_log_likelihood
                  - proposal_log_likelihood);
}

template <class ControlModel, class ObservationModel>
auto slam::fastslam<ControlModel, ObservationModel>::particle_log_weight(
    const particle_type& particle) const -> double {
  ...
  for (const auto& obs : seen_features) {
    ...
    unscented_transform(ukf_params.feature, typename vec::feature_observer(
                                                particle.trajectory.state),
                        feature.vector_model(), predicted_obs,
                        obs.observation.derived().chol_cov());
...
template <class ControlModel, class ObservationModel>
auto slam::fastslam<ControlModel, ObservationModel>::get_state(
    timestep_type timestep) const -> state_type {
  assert(timestep <= current_timestep());
  if (!discard_history && trajectory_estimate.size() != current_timestep()) {
    const typename particle_type::state_list* p =
        &particles.max_weight_particle().trajectory;
...
template <class ControlModel, class ObservationModel>
auto slam::fastslam<ControlModel, ObservationModel>::get_trajectory() const
...
template <class ControlModel, class ObservationModel>
auto slam::fastslam<ControlModel, ObservationModel>::get_feature_map() const
...
template <class ControlModel, class ObservationModel>
auto slam::fastslam<ControlModel, ObservationModel>::program_options()
    -> boost::program_options::options_description {
  namespace po = boost::program_options;
  po::options_description options("FastSLAM 2.0 Parameters");
  options.add_options() //
      ("num-particles", po::value<size_t>()->default_value(100),
       "Number of particles in the particle filter") //
      ("resample-threshold", po::value<double>()->default_value(0.75),
       "Minimum ratio of effective particles") //
      ("resample-threshold-min", po::value<double>()->default_value(0.5),
       "Minimum ratio before filter collapses")                        //
      ("no-history", "Don't keep per-particle trajectory information") //
      ("ukf-alpha", po::value<double>()->default_value(0.002),
       "The alpha parameter for the scaled UKF") //
      ("ukf-beta", po::value<double>()->default_value(2),
       "The beta parameter for the scaled UKF") //
      ("ukf-kappa", po::value<double>()->default_value(0),
       "The kappa parameter for the scaled UKF") //
      ("fastslam-seed", po::value<unsigned int>(), "FastSLAM 2.0 random seed");
  return options;
}

template <class ControlModel, class ObservationModel>
slam::fastslam<ControlModel, ObservationModel>::fastslam(
    boost::program_options::variables_map& options, unsigned int seed)
    : random(seed), num_particles(options["num-particles"].as<size_t>()),
...
extern template class slam::fastslam<control_model_type,
