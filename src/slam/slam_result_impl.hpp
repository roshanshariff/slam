...
public:
  ...
  void set_initial_state(const state_type& state) { m_initial_state = state; }

  auto get_trajectory() -> trajectory_type& { return m_trajectory; }

  auto get_feature_map() -> feature_map_type& { return m_map; }
};

template <class ControlModel, class ObsModel>
using slam_result_of_impl =
    slam_result_impl<typename ControlModel::associated_type,
