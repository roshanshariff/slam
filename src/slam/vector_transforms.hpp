...
struct vector_transform_functors {
  ...
  struct state_feature_observer {
    observation_vector_type
    operator()(const state_feature_vector_type& vec) const {
      state_type state =
          state_type::from_vector(vec.template head<state_dim>());
      feature_type feature =
          feature_type::from_vector(vec.template tail<feature_dim>());
  ...
  struct feature_observer {
    ...
    observation_vector_type
    operator()(const feature_vector_type& feature) const {
      return ObservationModel::observe(-state
                                       + feature_type::from_vector(feature));
    }
  };
