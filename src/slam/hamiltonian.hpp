...
namespace slam {
    ...
    template <class Model> struct parametrization {
        ...
        static auto apply (const type& x) -> vector_type {
            return Model::observe(x);
        }
        ...
        static auto grad_log_likelihood (const Model& model, const type& x) -> vector_type {
    ...
    struct parametrization<planar_robot::range_only_model>
        ...
        static auto grad_log_likelihood (const planar_robot::range_only_model& model, const type& pos)
            -> vector_type
        {
            return model.grad_log_likelihood(planar_robot::range_only_model::observe(pos))(0)
                * (inv_jacobian(apply(pos)).transpose() * pos.to_vector().normalized());
        }

        static auto log_likelihood (const planar_robot::range_only_model& model, const type& pos)
            -> double
        {
        ...
        -> feature_timestep_map
    {
        ...
        for (const auto& t_obs : data.observations()) {
            ...
            if (anchor.find(id) == anchor.end()
                || obs.observation().more_accurate_than(data.get_observations(id).at(anchor[id]))) {
                anchor[id] = t;
            }
        }
        return anchor;
    ...
    template <class ControlModel, class ObservationModel>
    Eigen::VectorXd sqrt_mass_reparam (const slam_data<ControlModel, ObservationModel>& data,
                                       const feature_timestep_map& anchor) {

        static constexpr int state_dim = ControlModel::associated_type::vector_dim;
        static constexpr int feature_dim = ObservationModel::associated_type::vector_dim;

        Eigen::VectorXd sqrt_mass (state_dim*data.current_timestep() + feature_dim*anchor.size());

        for (timestep_type t; t < data.current_timestep(); ++t) {
            sqrt_mass.segment<state_dim>(t*state_dim) = data.control(t).chol_cov_diag();
        }

        std::size_t feature_segment_idx = state_dim * data.current_timestep();

        for (const auto& id_anchor : anchor) {
            const auto& obs_model = data.get_observations(id_anchor.first).at(id_anchor.second);
            const double r_sigma = obs_model.chol_cov_diag()(0);
            const double theta_sigma = r_sigma / (obs_model.mean()(0) + 1);
            sqrt_mass.segment<feature_dim>(feature_segment_idx) = Eigen::Vector2d(r_sigma, theta_sigma);
    ...
    template<class ControlModel, class ObservationModel>
    void update_state_reparam (const feature_timestep_map& anchor,
        ...
        assert(update.size() == state_dim*trajectory.size() + feature_dim*anchor.size());

        for (timestep_type t; t < trajectory.size(); ++t) {
            trajectory[t] = StateParam::inv(StateParam::apply(trajectory[t])
    ...
    template<class ControlModel, class ObservationModel>
    double reparam_log_likelihood (const slam_data<ControlModel, ObservationModel>& data,
        ...
        for (timestep_type t; t < estimate.current_timestep(); ++t) {
            log_likelihood += StateParam::log_likelihood(data.control(t), estimate.get_trajectory()[t]);
        }
    ...
    template <class ControlModel, class ObservationModel>
    void reparam_gradient (const slam_data<ControlModel, ObservationModel>& data,
                           const feature_timestep_map& anchor,
                           const slam_result_of<ControlModel, ObservationModel>& estimate,
        ...
        assert(gradient.size() == state_dim*estimte.current_timestep() + feature_dim*anchor.size());

        for (timestep_type t; t < estimate.current_timestep(); ++t) {
            auto grad = StateParam::grad_log_likelihood(data.control(t), estimate.get_trajectory()[t]);
        ...
        for (const auto& id_feature : estimate.get_feature_map()) {
            ...
            const auto compute_grad = [&estimate, &gradient, &feature](auto observations, auto timesteps) -> feature_vector_type {

                feature_vector_type accum_grad; accum_grad.setZero();
                ...
                for (; !timesteps.empty(); timesteps.drop_front()) {
                    ...
                    for (; !observations.empty() && observations.front().first == t; observations.drop_front()) {
                        accum_grad += FeatureParam::jacobian(rel_feature).transpose()
                            * FeatureParam::grad_log_likelihood(observations.front().second, rel_feature);
                    }

                    const auto state_delta = estimate.get_trajectory().accumulate(t, next_t);
                    const auto new_rel_feature = -state_delta + rel_feature;

                    const double theta_grad = FeatureParam::inv_jacobian(FeatureParam::apply(new_rel_feature))
                        .col(1).dot(accum_grad);
                    const state_vector_type grad = StateParam::inv_jacobian(StateParam::apply(state_delta)).transpose()
            ...
            feature_vector_type feature_grad; feature_grad.setZero();
            feature_grad += compute_grad(obs_range,
                                         boost::irange<std::size_t>(0, anchor_t));
            feature_grad += compute_grad(obs_range | boost::adaptors::reversed,
                                         boost::irange<std::size_t, int>(estimate.current_timestep(), anchor_t, -1));

            const auto rel_feature = -estimate.get_state(anchor_t) + feature;
            feature_grad = FeatureParam::inv_jacobian(FeatureParam::apply(rel_feature)).transpose()
                * feature_grad;
            feature_grad += FeatureParam::grad_log_likelihood(observations.at(anchor_t), rel_feature);
    ...
    class reparam_hmc : public slam_result_of_impl<ControlModel, ObservationModel> {
        ...
        using slam_result_impl_type = slam_result_of_impl<ControlModel, ObservationModel>;
    ...
    public:
        ...
        reparam_hmc (const decltype(data)& data,
                     const slam_result_type& initial_state,
                     unsigned int seed)
            : slam_result_impl_type(initial_state), data(data), anchor(find_feature_anchors(*data)),
              sqrt_mass_diag(sqrt_mass_reparam(*data, anchor)), random(seed)
        {
            for (auto& id_feature : this->get_feature_map()) {
                ...
                const auto init = data->get_observations(id).at(anchor_t).proposal().initial_value(random);
        ...
        bool update (const double epsilon, const int num_steps) {
            ...
            for (int step = 0; step < num_steps; ++step) {
                ...
                update_state_reparam<ControlModel, ObservationModel>(anchor, state, p, epsilon);
            ...
            std::cout << "T: " << this->current_timestep() << "N: " << this->get_feature_map().size() << '\n';

            const double current_U = -reparam_log_likelihood(*data, *this);
            const double proposed_U = -reparam_log_likelihood(*data, state);

            const double current_K = (sqrt_mass_diag.asDiagonal()*current_p).squaredNorm();
            const double proposed_K = (sqrt_mass_diag.asDiagonal()*p).squaredNorm();

            const double accept_prob = std::exp(current_U - proposed_U + current_K - proposed_K);
            std::cout << "current U: " << current_U
                      << ", proposed U: " << proposed_U
                      << ", current K: " << current_K
                      << ", proposed K: " << proposed_K << std::endl;
