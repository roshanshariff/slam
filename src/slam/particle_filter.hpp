...
public:
  ...
  particle_filter()
      : particles(1), max_weight(&particles.front()), weight_sum(1.0),
        squared_weight_sum(1.0) {}

  std::size_t size() const { return particles.size(); }

  double effective_size() const {
    return squared_weight_sum > 0 ? weight_sum * weight_sum / squared_weight_sum
                                  : 0;
  }
  ...
  void reinitialize(std::size_t new_size, Initializer);
...
template <class Particle>
void slam::particle_filter<Particle>::resample(random_source& random,
                                               std::size_t new_size) {
...
template <class Particle>
template <class Initializer>
void slam::particle_filter<Particle>::reinitialize(std::size_t new_size,
                                                   Initializer init) {
  particles.clear();
