...
public:
  multi_mcmc(std::shared_ptr<const slam_data_type>,
  ...
  auto num_chains() const -> unsigned int { return mcmc_chains.size(); }
  ...
  virtual void timestep(timestep_type t) override {
    for (const auto& mcmc : mcmc_chains) { mcmc->timestep(t); }
  }
...
template <class ControlModel, class ObservationModel>
void slam::multi_mcmc<ControlModel, ObservationModel>::update(
    unsigned int count) {
...
template <class ControlModel, class ObservationModel>
void slam::multi_mcmc<ControlModel, ObservationModel>::completed() {
  ...
  while (remaining_steps > 0) {
    ...
    report << ((double)num_updates / num_chains()) << '\t'
...
template <class ControlModel, class ObservationModel>
auto slam::multi_mcmc<ControlModel, ObservationModel>::program_options()
    -> boost::program_options::options_description {
  namespace po = boost::program_options;
  po::options_description options("Multi-MCMC Parameters");
  options.add_options() //
      ("multi-mcmc-chains", po::value<unsigned int>()->default_value(100),
       "Number of MCMC chains") //
      ("multi-mcmc-end-steps", po::value<unsigned int>()->default_value(0),
       "MCMC iterations after simulation");
  return options;
}

template <class ControlModel, class ObservationModel>
slam::multi_mcmc<ControlModel, ObservationModel>::multi_mcmc(
  ...
  unsigned int num_mcmc_chains =
      options["multi-mcmc-chains"].as<unsigned int>();
...
extern template class slam::multi_mcmc<control_model_type,
