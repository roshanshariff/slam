...
public:
  ...
  virtual auto observations_at(timestep_type t) const
      -> observation_range override {
  ...
  void add_observation(timestep_type t, featureid_type id,
                       observation_type obs) {
    m_observations.emplace(t, observation_info{id, obs});
  }

  void add_observation_now(featureid_type id, observation_type obs) {
    m_observations.emplace_hint(m_observations.cend(), current_timestep(),
                                observation_info{id, obs});
  }
};
