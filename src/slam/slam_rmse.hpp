...
auto rms_error(const slam_result<State, Feature>& a,
               const slam_result<State, Feature>& b) -> double {
...

  {
    ...
    for (size_t i = 1; i <= t; ++i) {
      acc(State::subtract(at.accumulate(i).to_vector(),
                          bt.accumulate(i).to_vector())
              .squaredNorm());
    }
  }
