...
public:
  ...
  static auto from_vector(const vector_type& v) -> result_type {
    return vector_model_type::from_vector(v);
  }
  static auto to_vector(const result_type& x) -> vector_type {
    return vector_model_type::to_vector(x);
  }
  static auto subtract(const vector_type& a, const vector_type& b)
      -> vector_type {
    return vector_model_type::subtract(a, b);
  }

  auto vector_model() const -> const vector_model_type& { return model; }
  auto vector_model() -> vector_model_type& { return model; }

  auto mean() const -> result_type {
    return from_vector(vector_model().mean());
  }

  auto operator()(random_source& rand) const -> result_type {
    return from_vector(vector_model()(rand));
  }

  auto likelihood(const result_type& x) const -> double {
    return vector_model().likelihood(to_vector(x));
  }

  auto log_likelihood(const result_type& x) const -> double {
    return vector_model().log_likelihood(to_vector(x));
  }

  struct builder
      : public std::unary_function<result_type, vector_model_adapter> {

    using vector_builder_type = typename vector_model_type::builder;
    vector_builder_type vector_builder;

    builder(const vector_builder_type& vector_builder_)
        : vector_builder(vector_builder_) {}

    builder(const boost::program_options::variables_map& options)
        : vector_builder(options) {}
