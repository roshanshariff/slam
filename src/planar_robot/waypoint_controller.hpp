...
public:
  ...
  auto control(const pose& state, double dt) ->
      typename ControlModel::vector_type;
...
public:
  pose initial_state() const {
    return waypoints.empty() ? pose()
                             : pose::from_position(waypoints.front(), 0.0);
  }

  bool finished() const { return waypoint >= waypoints.size() * repetitions; }
...
template <class ControlModel>
auto planar_robot::waypoint_controller::control(const pose& state,
                                                const double dt) ->
    typename ControlModel::vector_type {
