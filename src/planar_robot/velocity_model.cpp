...
auto velocity_slip_model::builder::program_options()
    -> po::options_description {

  const double RAD_PER_DEG = boost::math::constants::pi<double>() / 180;

  po::options_description model_options(
      "Velocity-Slip Motion Model Parameters");
  model_options.add_options() //
      ("velmodel-alpha1",
       po::value<double>()->default_value(0.01))                   //
      ("velmodel-alpha2", po::value<double>()->default_value(0.0)) //
      ("velmodel-alpha3",
       po::value<double>()->default_value(1.0 * RAD_PER_DEG * RAD_PER_DEG)) //
      ("velmodel-alpha4", po::value<double>()->default_value(0.01))         //
      ("velmodel-alpha5",
       po::value<double>()->default_value(0.01 * RAD_PER_DEG * RAD_PER_DEG)) //
      ("velmodel-alpha6",
       po::value<double>()->default_value(0.001 * RAD_PER_DEG));
