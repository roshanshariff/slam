...
auto planar_robot::waypoint_controller::program_options()
  ...
  robot_options.add_options() //
      ...
      ("robot-max-steering",
       po::value<double>()->default_value(30.0 * RAD_PER_DEG),
       "maximum steering angle, in radian/second") //
      ("robot-max-steering-rate",
       po::value<double>()->default_value(20.0 * RAD_PER_DEG),
...
planar_robot::waypoint_controller::waypoint_controller(
  ...
  if (options.count("waypoint-file")) {
    ...
    std::ifstream waypoint_file(
        options["waypoint-file"].as<std::string>().c_str());
    while (waypoint_file >> x >> y)
      waypoints.push_back(position::cartesian(x, y));
  }
}
