...
struct range_bearing_model
    : public independent_normal_base<2, range_bearing_model> {
  ...
  public:
    builder(double range_stddev, double bearing_stddev)
        : stddev(range_stddev, bearing_stddev) {}
    builder(const boost::program_options::variables_map&);

    static auto program_options()
        -> boost::program_options::options_description;

    auto operator()(const vector_type& observation) const
        -> range_bearing_model {
...
struct range_only_model : public independent_normal_base<1, range_only_model> {
  ...
  range_only_model(const vector_type& mean, const vector_type& stddev)
      : base_type(mean, stddev) {}
  ...
  public:
    ...
    auto operator()(random_source& random) const -> result_type {
      ...
      return position::polar(model(random)(0),
                             random.uniform() * 2 * pi<double>());
    }
    ...
    auto initial_value(random_source& random) const -> result_type {
      using namespace boost::math::constants;
      return position::polar(model.mean()(0),
                             random.uniform() * 2 * pi<double>());
    }
  };
  ...
  static auto learn_from_data(
      const slam::dataset<ControlModel, range_only_model>& dataset,
      const slam::slam_result_of<ControlModel, range_only_model>& ground_truth)
      -> builder;
};

template <class ControlModel>
auto range_only_model::learn_from_data(
    const slam::dataset<ControlModel, range_only_model>& dataset,
    const slam::slam_result_of<ControlModel, range_only_model>& ground_truth)
    -> builder {
